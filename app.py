...
if not st.session_state['utente_loggato']:
    ...
    with tab_reg:
        ...
        email = st.text_input("Email universitaria") # Etichetta aggiornata
...
# --- APP PRINCIPALE ---
    ...
    if is_direttivo:
        # Rinominato Documenti Membri -> Anagrafiche Membri vista la rimozione dei documenti personali
        opzioni_menu.extend(["📄 Scadenze Documenti", "📂 Anagrafiche Membri", "🛒 Gestione Acquisti", "📊 Bilancio Budget", "⚙️ Gestione Utenti"])
    ...
    # ==========================================
    # 1. HOME CON DASHBOARD E MINI CALENDARIO
    # ==========================================
    if menu == "🏠 Home":
    ...
    # ==========================================
    # 4. TASK MANAGER E 5. TEST & PISTA ...
    # (Inserisco qui i blocchi per mantenere tutte le funzioni)
    # ==========================================
        ...
        try:
            ...
            st.info("Tabella task in configurazione.")
        ...
        with st.expander("➕ Registra Nuova Sessione di Test"):
            with st.form("form_test"):
                ...
                if st.form_submit_button("Salva Sessione Test"):
                    if luogo_test:
                        url_tel = upload_file_to_supabase(file_telemetria.getvalue(), file_telemetria.name, "workspaces") if file_telemetria else None
                        supabase.table("sessioni_test").insert({
                            "data_test": str(d_test), "luogo": luogo_test, "vettura": vettura_test, "condizioni_meteo": meteo,
                            "pilota": pilota, "note_set_up": note_setup, "miglior_tempo": miglior_tempo, "url_telemetria": url_tel, "creato_da": utente['username']
                        }).execute()
        ...
        try:
            ...
            st.info("Tabella test in configurazione.")

    # ==========================================
    # 6. DIZIONARIO COMPONENTI E BOM
    # ==========================================
    elif menu == "📖 Dizionario Componenti":
        st.title("📖 Dizionario Componenti MMR")
        st.write("Consulta o inserisci i componenti della vettura, la loro funzione e immagini.")
        
        with st.expander("➕ Inserisci Nuovo Componente"):
            with st.form("form_dizionario"):
                nome_comp = st.text_input("Nome Componente")
                divisione_comp = st.selectbox("Divisione", ["Business", "Powertrain CV", "Powertrain EV", "Frame and Body", "Aerodynamics and Thermal Managment", "Vehicle Dynamics", "Electronics", "Actuations", "Autonomous Driving", "Marketing", "Manufacturing", "Communication"])
                descrizione_comp = st.text_area("Cosa fa e specifiche tecniche")
                foto_comp = st.file_uploader("Carica Foto (Opzionale)", type=["png", "jpg", "jpeg"])
                
                if st.form_submit_button("Salva nel Dizionario"):
                    if nome_comp:
                        url_foto = upload_file_to_supabase(foto_comp.getvalue(), foto_comp.name, "workspaces") if foto_comp else None
                        supabase.table("dizionario_componenti").insert({"nome": nome_comp, "divisione": divisione_comp, "descrizione": descrizione_comp, "url_foto": url_foto, "creato_da": utente['username']}).execute()
                        st.success("Aggiunto!")
                        st.rerun()
                    else:
            ...
            if componenti:
                for c in componenti:
                    with st.expander(f"⚙️ {c['nome']} — *[{c['divisione']}]*"):
                        st.write(f"**Funzione:** {c['descrizione']}")
                        st.caption(f"Inserito da: {c.get('creato_da', 'N/D')}")
                        if c.get('url_foto'): st.image(c['url_foto'], width=400)
            else:
                st.info("Nessun componente.")
        except:
            st.info("Tabella dizionario in configurazione.")
        ...
        with st.expander("➕ Aggiungi Elemento alla BOM"):
            with st.form("form_bom"):
                ...
                gruppo_assieme = c3.selectbox("Sottoassieme", ["Telaio e Aerodinamica", "Powertrain / Motore", "Elettronica & Ecu", "Sospensioni e Dinamica", "Actuations & Pneumatica", "Altro"])
                quantita = c4.number_input("Quantità", min_value=1, value=1)
                divisione_rif = c5.selectbox("Divisione", ["Business", "Powertrain CV", "Powertrain EV", "Frame and Body", "Aerodynamics and Thermal Managment", "Vehicle Dynamics", "Electronics", "Actuations", "Autonomous Driving", "Marketing", "Manufacturing", "Communication"])
                materiale = st.text_input("Materiale / Specifiche")
                ...
                if st.form_submit_button("Aggiungi"):
                    if codice_parte and nome_parte:
                        supabase.table("bom_vettura").insert({"codice": codice_parte, "nome": nome_parte, "assieme": gruppo_assieme, "quantita": quantita, "divisione": divisione_rif, "materiale": materiale, "stato": stato_produzione, "aggiornato_da": utente['username']}).execute()
                        st.success("Aggiunto!")
                        st.rerun()
                    else:
        ...
        try:
            ...
            if bom_data:
                df_bom = pd.DataFrame(bom_data)[['codice', 'nome', 'assieme', 'quantita', 'divisione', 'materiale', 'stato', 'aggiornato_da']]
                df_bom.columns = ['Codice', 'Nome Componente', 'Assieme', 'Qtà', 'Divisione', 'Materiale', 'Stato', 'Agg. da']
                st.dataframe(df_bom, use_container_width=True, hide_index=True)
            else:
                st.info("BOM vuota.")
        except:
            st.info("Tabella BOM in configurazione.")

    # ==========================================
    # 7. AREA PERSONALE (Solo dati anagrafici)
    # ==========================================
    elif menu == "👤 Area Personale":
        st.title(f"👤 Area Personale di {utente['nome']} {utente['cognome']}")
        st.write("Inserisci i tuoi dati anagrafici essenziali.")
        ...
        st.markdown(
            ...
            I dati anagrafici raccolti sono trattati in conformità al GDPR dal team <b>More Modena Racing</b> 
            per le sole finalità istituzionali.
            </div>""", unsafe_allow_html=True
        )

    # ==========================================
    # 8. RUBRICA CONTATTI (Email Rimossa)
    # ==========================================
    elif menu == "👥 Rubrica Contatti":
        st.title("👥 Rubrica Contatti")
        utenti_db = supabase.table("membri").select("nome, cognome, telefono, ruolo, divisione").execute().data
        if utenti_db:
            df = pd.DataFrame(utenti_db)
            # Nascondiamo la mail per privacy interna, mostriamo il telefono se presente
            df.columns = ['Nome', 'Cognome', 'Telefono', 'Ruolo', 'Divisione']
            st.dataframe(df, use_container_width=True, hide_index=True)
        else:
            st.info("Nessun membro trovato.")

    # ==========================================
    # 9. SCADENZE DOCUMENTI
    # ==========================================
        ...
        with st.expander("➕ Aggiungi Nuova Scadenza con Template"):
            with st.form("form_scadenza"):
                ...
                if st.form_submit_button("Crea Scadenza"):
                    if nome_doc:
                        url_temp = upload_file_to_supabase(file_template_nuovo.getvalue(), file_template_nuovo.name, "workspaces") if file_template_nuovo else None
                        supabase.table("scadenze_documenti").insert({"nome_documento": nome_doc, "data_scadenza": str(data_scad), "note": note_doc, "url_template": url_temp, "creato_da": utente['username']}).execute()
        ...
        if scadenze_esistenti:
            for s in scadenze_esistenti:
                with st.expander(f"🚨 {s['nome_documento']} - Scadenza: {formatta_data_it(s['data_scadenza'])}"):
                    ...
                    if s.get('url_template'): st.markdown(f"📄 **[Scarica Template]({s['url_template']})**")
                    if st.button("🗑️ Elimina", key=f"del_scad_{s['id']}"):
                        supabase.table("scadenze_documenti").delete().eq("id", s['id']).execute()
                        st.rerun()
        else:
            st.info("Non ci sono scadenze attive.")

    # ==========================================
    # 10. ANAGRAFICHE MEMBRI (RISERVATO AL BOARD)
    # ==========================================
    elif menu == "📂 Anagrafiche Membri":
        st.title("📂 Anagrafiche Membri")
        st.write("Sezione riservata al Board: visualizza i dati personali di tutti i membri in ordine alfabetico.")
        ...
        if tutti_membri:
            for m in tutti_membri:
                ...
                with st.expander(f"👤 {nome_completo}"):
                    ...
                    c_d1.write(f"**Email Univ.:** {m.get('email', '-')}")
            ...
            if richieste:
                for req in richieste:
                    with st.expander(f"[{req['divisione']}] {req['oggetto']} ({req['stato']})"):
                        ...
                        st.write(f"**Costo:** € {req['costo_no_iva']} | **IVA Applicabile:** {'Sì' if req.get('iva_applicabile', True) else 'No'}")
                        ...
                        if req.get('link_dichiarazione_unicita'): st.markdown(f"📄 [Dichiarazione Unicità]({req['link_dichiarazione_unicita']})")
                        
                        c1, c2 = st.columns(2)
                        with c1: nuovo_stato = st.selectbox("Stato", stati_acquisto, index=stati_acquisto.index(req['stato']), key=f"stato_{req['id']}")
                        with c2: nuovo_fondo = st.text_input("Fondo", value=req['fondo'] if req['fondo'] else "", key=f"fondo_{req['id']}")
                        
                        col_salva, col_elimina = st.columns(2)
                        if col_salva.button("💾 Salva Modifiche", key=f"btn_salva_{req['id']}"):
                            supabase.table("acquisti").update({"stato": nuovo_stato, "fondo": nuovo_fondo}).eq("id", req['id']).execute()
                            st.rerun()
                        if col_elimina.button("🗑️ Elimina", key=f"btn_elimina_{req['id']}"):
                            supabase.table("acquisti").delete().eq("id", req['id']).execute()
                            st.rerun()
            else:
                st.info("Nessun acquisto in sospeso.")
                
        if is_direttivo:
            st.subheader("Inoltra Nuova Richiesta")
            oggetto = st.text_input("Oggetto")
            azienda = st.text_input("Azienda")
            costo = st.number_input("Costo (Senza IVA)", min_value=0.0)
            iva_applicabile = st.checkbox("Applica IVA (22%)", value=True)
            unica_prod = st.checkbox("Azienda unica produttrice?")
            
            file_p1 = st.file_uploader("Prev 1", type=["pdf"])
            file_p2 = st.file_uploader("Prev 2 (se non unica)", type=["pdf"]) if not unica_prod else None
            file_p3 = st.file_uploader("Prev 3 (se >5000€)", type=["pdf"]) if not unica_prod and costo > 5000 else None
            file_dichiarazione = st.file_uploader("Dichiarazione Unicità", type=["pdf"]) if unica_prod else None
            
            if st.button("📤 Inoltra al CFO"):
                if oggetto and azienda and costo > 0:
                    with st.spinner("Invio..."):
                        u1 = upload_file_to_supabase(file_p1.getvalue(), file_p1.name, "preventivi") if file_p1 else None
                        u2 = upload_file_to_supabase(file_p2.getvalue(), file_p2.name, "preventivi") if file_p2 else None
                        u3 = upload_file_to_supabase(file_p3.getvalue(), file_p3.name, "preventivi") if file_p3 else None
                        ud = upload_file_to_supabase(file_dichiarazione.getvalue(), file_dichiarazione.name, "preventivi") if file_dichiarazione else None
                        supabase.table("acquisti").insert({"richiedente": utente['username'], "divisione": utente['divisione'], "oggetto": oggetto, "azienda": azienda, "costo_no_iva": costo, "iva_applicabile": iva_applicabile, "unica_produttrice": unica_prod, "link_preventivo_1": u1, "link_preventivo_2": u2, "link_preventivo_3": u3, "link_dichiarazione_unicita": ud, "stato": "Inoltrato"}).execute()
                        st.success("Inviato!")
                        st.rerun()
                else: st.error("Compila Oggetto, Azienda e Costo.")
            ...
            if risposta_mie:
                st.dataframe(pd.DataFrame(risposta_mie)[['oggetto', 'azienda', 'costo_no_iva', 'stato', 'fondo']], use_container_width=True, hide_index=True)
    ...
    # ==========================================
        ...
        if risposta_bilancio:
            ...
            st.metric(label="Totale Spese Approvate (IVA inclusa dove applicabile)", value=f"€ {df_bilancio['Costo Finanziato'].sum():,.2f}")
            ...
            for u in utenti:
                if u['username'] != utente['username']:
                    with st.container():
                        ...
                        if col_salva.button("🔄 Aggiorna", key=f"aggiorna_{u['id']}"):
                            ...
                            if utente['ruolo'] in ["Division Leader", "CTO"] and u['ruolo'] != "Membro":
                                st.error("❌ Solo sui Membri.")
                            elif is_downgrade and utente['ruolo'] != "Team Leader":
                                st.error("❌ Solo il TL può degradare.")
                            else:
                                supabase.table("membri").update({"ruolo": nuovo_ruolo}).eq("id", u['id']).execute()
                                st.rerun()
                                
                        if col_elimina.button("🗑️", key=f"elimina_{u['id']}"):
                            if utente['ruolo'] in ["Division Leader", "CTO"] and u['ruolo'] != "Membro":
                                st.error("❌ Non puoi eliminare direttivi.")
                            else:
